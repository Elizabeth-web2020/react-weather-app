...
function App() {
  ...
  const getWeather = async (location) => {
    ...
    try {
      ...
      let res = await fetch(`${process.env.REACT_APP_URL + howToSearch}&appid=${process.env.REACT_APP_API_KEY}&units=metric&cnt=5&exclude=hourly,minutely`)
      console.log(res)
  ...
  return (
    <div className="bg-gray-800 flex items-center justify-center w-screen h-screen py-10">
      <div className="flex w-3/4 h-full rounded-3xl shadow-lg m-auto bg-gray-100"> 
        {/* form card section */}
        <div className="form-container">
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <form noValidate onSubmit={handleSubmit} className="flex justify-center w-full">
              <input 
                type="text"
                className="relative focus:outline-none rounded-l-xl py-2 px-3 w-2/3 bg-gray-300 bg-opacity-60 text-white placeholder-gray-200"
                onChange={handleChange}
                required
              />
              <button type="submit" className="z-10">
                <i className="fa fa-search text-white border-1 my-auto z-10 bg-gray-300 bg-opacity-60 cursor-pointer p-3 rounded-r-xl"
        ...
        <div className="w-2/4 p-5">
          ...
          <div className="flex flex-col my-10">
            ...
            <>
              <h1 className="text-4xl text-gray-800 mt-auto mb-4">Today</h1>
              <DetailCard weather_icon={weatherIcon} data={weatherData}/>
              <h1 className="text-2xl text-gray-600 mb-4 mt-6">More On {city}</h1>
